...
Use the same runner for broader smoke captures:

```bash
CODEXBAR_PROOF_CAPTURE_MODE=menu scripts/run_vm_provider_proof.sh codex
CODEXBAR_PROOF_CAPTURE_MODE=popout scripts/run_vm_provider_proof.sh codex
CODEXBAR_PROOF_CAPTURE_MODE=tab CODEXBAR_PROOF_PREFERENCES_TAB=about scripts/run_vm_provider_proof.sh codex
CODEXBAR_PROOF_CAPTURE_MODE=tab CODEXBAR_PROOF_PREFERENCES_TAB=accounts scripts/run_vm_provider_proof.sh codex
```
...
The proof scripts now enforce several of these automatically:

- **State JSON validation**: `run_vm_provider_proof.sh` rejects proofs where the state
  JSON is empty, missing `preferences_viewport_outer_rect`, or where the
  `preferences_tab` field doesn't match the requested tab.
- **Brightness checks**: Host captures, guest captures, and cropped outputs are all
  brightness-validated. The guest capture threshold was raised from 0.01 to 0.15 to
  reject near-black images. Cropped output below 0.05 mean triggers a warning.
- **Ready-marker timeout**: If the guest ready marker doesn't appear within 120s, a
  warning is emitted to stderr instead of silently continuing.
- **Crop output validation**: The crop script now verifies the output file is non-empty
  and warns if the cropped region is suspiciously dark.

These checks can reject proofs that _look_ like they succeeded (file exists, non-zero
size) but actually contain stale or black content. A `PROOF REJECTED:` message on
stderr means the proof should not be trusted.
