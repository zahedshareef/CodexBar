...
function Remove-StaleBuiltCodexBarArtifacts {
  ...
  $debugDir = Join-Path $RepoRoot 'rust\target\x86_64-pc-windows-msvc\debug'
  $paths = @(
...
function Invoke-InteractiveCommandTask {
  ...
  $principal = New-ScheduledTaskPrincipal -UserId 'mac' -LogonType Interactive -RunLevel Highest
...
$action = New-ScheduledTaskAction -Execute 'C:\Users\mac\src\Win-CodexBar\rust\target\x86_64-pc-windows-msvc\debug\codexbar.exe' -Argument 'menubar'
$trigger = New-ScheduledTaskTrigger -Once -At ((Get-Date).AddMinutes(1))
$principal = New-ScheduledTaskPrincipal -UserId 'mac' -LogonType Interactive -RunLevel Highest
Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Principal $principal -Force | Out-Null
Start-ScheduledTask -TaskName $taskName
...
Send-TestCommand -Line '{"type":"select_preferences_tab","tab":"accounts"}'
