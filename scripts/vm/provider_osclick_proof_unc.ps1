...
if ($CaptureMode -eq 'popout') {
  ...
  try {
    $nircmd = Ensure-NirCmd
    $captureTask = ('CodexBarProof-' + ($ProofName -replace '[^A-Za-z0-9_-]', '-') + '-popout-capture')
    Invoke-InteractiveCommandTask `
      -TaskName $captureTask `
      -Execute $nircmd `
    ...
    if (Test-Path $interactiveDesktopShot) {
      Log-WindowCapture 'nircmd_interactive_saved_popout_full_desktop_shot'
      Get-Item $interactiveDesktopShot | Select-Object FullName, Length, LastWriteTime
    }
  } catch {
    Log-WindowCapture ("popout_nircmd_failed=" + $_.Exception.Message)
  }
